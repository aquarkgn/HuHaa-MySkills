...
> 本文档定义 SkillHelper 的技能扫描规范，是稳定的工程规则，供扫描器（`packages/scanner`）与服务端（`packages/server`）遵循。
...
- 缓存目录：`~/.config/skillhelper/icon-cache/`（受 `SKILLHELPER_HOME` / `XDG_CONFIG_HOME` 影响）
