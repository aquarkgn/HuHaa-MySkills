#!/bin/bash
# HuHaa-MySkills Data Source Diagnostic and Activation Script
...
echo ""
echo "╔════════════════════════════════════════════════════════════╗"
echo "║  HuHaa-MySkills 数据源诊断和激活工具                      ║"
...
# MCP activation
echo "┌─ MCP Config 激活"
if grep -q "~/mcp/dingtalk.mcp.json" ~/.config/huhaa-myskills/sources.yaml 2>/dev/null; then
  ...
  if [ "$DRY_RUN" = "false" ]; then
    ...
    if [ -f ~/.config/huhaa-myskills/sources.yaml ]; then
      cp ~/.config/huhaa-myskills/sources.yaml ~/.config/huhaa-myskills/sources.yaml.bak
